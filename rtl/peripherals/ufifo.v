...
module ufifo #(
	...
	// w_fill is unknown (X) upon the startup, and doesin't change until any UART RX transaction
	// hapens. This leads to FIFO register being in unknown (X) status, which causes ibex core
	// errors, then trying to read it. The best workaround for now, is just to use assign statment, 
	// not "always @(*)" block (which, again, doesin't happen until any transaction)
