...
/*
*  Fixed-priority arbitration wbm[0] has top priority
*  Grants bus access based on cyc, one master accesses the bus at a time
*
*/
module wb_interconnect_sharedbus
  #(parameter numm = 2,                      // number of masters
    // parameter nums = 3,                      // number of slaves
    parameter nums = 6, //bug of verilator http://github.com/verilator/verilator/issues/5890
    parameter [31:0] base_addr[nums] = '{default: 0}, // base addresses of slaves
    parameter [31:0] size[nums]      = '{default: 0}) // address size of slaves
...
`endif
   ...
   always_comb
     for (int i = 0; i < nums; i++)
       ss[i] = (adr >= base_addr[i]) && (adr < base_addr[i] + size[i]); // Slave select if in slave address bounds

   /* Assume, that the response is exactly on cycle after request. */
   always_ff @(posedge wbs[0].clk or negedge wbs[0].rst)
     if (!wbs[0].rst)
       ss1 <= '0;
     else
       if (cyc && stb)
         ss1 <= ss;     // ss1 is asserted if WBm asserts cyc and stb 

   /* priority arbiter */
   always_comb begin
      gnt = '0;
      for (int i = 0; i < numm; i++)   // give gnt to the lowest index master that asserts cyc
        if (wbm_cyc[i]) begin
           gnt[i] = 1'b1;
           break;
        end
   ...
   /* Assume, that the response is exactly on cycle after request. */
   always_ff @(posedge wbm[0].clk or negedge wbm[0].rst)
     if (!wbm[0].rst)
       gnt1 <= '0;
     else
       if (cyc && stb)
         gnt1 <= gnt;   // gnt1 is asserted if a master has gnt and has asserted cyc and stb
   ...
   always_comb begin
      for (int i = 0; i < numm; i++) begin
         wbm_stall[i] = 1'b1;       // All masters are stalled
         if (gnt[i])
           wbm_stall[i] = stall;    // If master has grant then connect slave stall
      end
   end
   ...
   always_comb begin
      ...
      `else
         wbm_dat_o[i] = '0;
      `endif
         if (gnt1[i]) begin      // If master has grant connect it to corresponding slave
            wbm_ack[i]   = ack;
            wbm_err[i]   = err;
         `ifdef NO_MODPORT_EXPRESSIONS
            wbm_dat_s[i] = dat_rd;  // Connect master input to slave output
         `else
            wbm_dat_o[i] = dat_rd;
         `endif
         end
   ...
   always_comb
     ...
     `else
        wbs_dat_o[i] = '0;
     `endif
        if (ss[i]) begin            // If slave is selected connect it to a master
           wbs_adr[i]   = adr;
           wbs_stb[i]   = cyc & stb;
           wbs_sel[i]   = sel;
        `ifdef NO_MODPORT_EXPRESSIONS
