 ...
 *
 * @param timer Timer Handle struct.
 */
...
void timer_irq_handler_callback(timer_handle *timer) __attribute__((weak));
...
/**
 * @brief Clears the timer interrupt flag (by writing 1 to bit 2) AND disables further timer interrupts.
 */
#define TIMER_ACK_INT_AND_DISABLE_IRQ(timer)                                              \
do {                                                                                      \
  (timer->regs->CTRL =  ((timer->regs->CTRL & ~TIMER_CTRL_ENA_INT) | TIMER_CTRL_FLAG));   \
} while (0)
