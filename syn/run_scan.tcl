...
if {[info exists UNGROUP_DESIGN] && $UNGROUP_DESIGN} {
    puts "Ungroup Enabled."
} else {
    puts "Ungroup disabled."
...
# 10 MHz test-clock; rise and fall times as default because the source clock is not a primary net
define_test_clock -name clk_test -period 100000 clk_sys_Pad 
...
define_test_mode -name test_mode -active high -lec_value 0 test_mode_Pad
...
if {[info exists UNGROUP_DESIGN] && $UNGROUP_DESIGN} {
    add_shadow_logic \
      -around ibex_simple_system/u_ibex_simple_system_int/imem_sram1 \
      -mode bypass \
      -balance \
...
if {[info exists UNGROUP_DESIGN] && $UNGROUP_DESIGN} {
    add_shadow_logic \
        -around ibex_simple_system/u_ibex_simple_system_int/imem_sram2 \
        -mode bypass \
        -balance \
        -test_control test_mode
} else {
...
if {[info exists UNGROUP_DESIGN] && $UNGROUP_DESIGN} {
  add_shadow_logic \
      -around ibex_simple_system/u_ibex_simple_system_int/dmem_sram \
      -mode bypass \
      -balance \
      -test_control test_mode
} else {
...
# set_db / .syn_opt_effort $INCR_OPT_EFF
# syn_opt -incremental
